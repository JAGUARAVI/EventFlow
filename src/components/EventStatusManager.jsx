...
const STATUS_PIPELINE = [
  { value: 'draft', label: 'Draft', color: 'default', description: 'Event is being set up' },
  {
  ...
  { value: 'live', label: 'Live', color: 'danger', description: 'Event is happening now' },
  { value: 'completed', label: 'Completed', color: 'success', description: 'Event has finished' },
  { value: 'archived', label: 'Archived', color: 'secondary', description: 'Event is archived' },
];
...
export default function EventStatusManager({ event, onStatusChange }) {
  const [isOpen, setIsOpen] = useState(false);
  const [loading, setLoading] = useState(false);
  const [selectedStatus, setSelectedStatus] = useState(null);

  const currentStatusObj = STATUS_PIPELINE.find((s) => s.value === event.status);
    ...
    <>
      <Dropdown>
        ...
        <DropdownMenu
          ...
          disabledKeys={new Set(
            STATUS_PIPELINE.filter((s) => !availableTransitions.includes(s.value)).map(
              (s) => s.value
            )
          )}
        >
          {STATUS_PIPELINE.map((status) => (
            <DropdownItem
              ...
              color={status.value === event.status ? status.color : 'default'}
            >
              {status.label}
            </DropdownItem>
          ))}
      ...
      <Modal isOpen={isOpen} onOpenChange={setIsOpen}>
        <ModalContent>
          ...
          <ModalBody>
            <p className="text-sm">
              Change event status from <strong>{currentStatusObj?.label}</strong> to{' '}
              <strong>
                {STATUS_PIPELINE.find((s) => s.value === selectedStatus)?.label}
              </strong>
              ?
            </p>

            {selectedStatus && getStatusRestrictions(selectedStatus).length > 0 && (
              <div className="bg-warning-50 border border-warning rounded p-3">
                <p className="text-sm font-semibold text-warning mb-1">Important:</p>
                <ul className="text-sm text-warning space-y-1">
                  {getStatusRestrictions(selectedStatus).map((r, i) => (
                    <li key={i}>• {r}</li>
                  ))}
                </ul>
              </div>
