    ...
    if (content.trim()) {
      try {
        const imported = metadataTemplate
          ? importTeamsWithMetadataFromCSV(content, metadataTemplate.fields_json || [])
                  ...
                  <CardBody className="gap-4">
                    <div>
                      <p className="text-sm text-default-600 mb-2">
                        Paste CSV content or upload a file. Required column: <strong>Team Name</strong>
                        <br />
                        Optional: Score, Description, {metadataTemplate?.fields_json?.length > 0 && 'and custom fields'}
                      </p>
                      ...
                      {importError && (
                        <div className="text-danger text-sm mt-2">{importError}</div>
                      )}
                    </div>

                    {importPreview.length > 0 && (
                      <div>
                        <h4 className="font-semibold mb-2">Preview ({importPreview.length} teams)</h4>
                        <Table isStriped>
                          <TableHeader>
                          ...
                          <TableBody>
                            {importPreview.slice(0, 10).map((team, idx) => (
                              <TableRow key={idx}>
