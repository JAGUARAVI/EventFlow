...
export default function EventPage() {
    ...
    const buildBracketMatches = (type) => {
        ...
        if (type === "single_elim")
            return generateSingleElimination(id, shuffledTeams);
        if (type === "round_robin")
            return generateRoundRobin(id, shuffledTeams);
    ...
    const handleRegenerateBracket = async () => {
        ...
        if (
            !confirm(
                "Regenerate bracket? This will delete all existing matches.",
            )
        )
            return;
    ...
    const handleDeletePoll = async (pollId) => {
        if (!confirm("Delete this poll? This will remove all its votes."))
            return;
    ...
    const handleDownloadPollCSV = async (poll) => {
        try {
            ...
            const pollWithOptions = {
                ...poll,
                options: poll.options || pollOptions[poll.id] || [],
            };
            const csv = exportPollsAnalysisToCSV(
                [pollWithOptions],
                votesData || [],
            );
    ...
    const handleDownloadAllPollsCSV = async () => {
        try {
            if (!polls || polls.length === 0) {
                addToast({
                    title: "No polls",
                    description: "There are no polls to export",
                    severity: "warning",
                });
                return;
            }
            ...
            const pollsWithOptions = polls.map((p) => ({
                ...p,
                options: p.options || pollOptions[p.id] || [],
            }));
            const csv = exportPollsAnalysisToCSV(
                pollsWithOptions,
                votesData || [],
    ...
    const finalizeSingleElimBracket = async () => {
        ...
        for (let i = 0; i < 6; i++) {
            ...
            current.forEach((m) => {
                if (m.next_match_id) {
                    currentFedInfo.add(
                        `${m.next_match_id}_${m.next_match_slot || "a"}`,
                    );
                }
            });
            ...
            <MatchEditor
                ...
                onAudit={async (match, update) => {
                    ...
                    const teamB =
                        ...
                        "Team B";
                    const winnerName =
                        teams.find((t) => t.id === update.winner_id)?.name ||
                        "TBD";
                    await supabase.from("event_audit").insert({
                        event_id: id,
                        action: "match.update",
                ...
                <ModalContent>
                    ...
                    <ModalBody className="space-y-4">
                        <div className="p-3 bg-danger/10 text-danger text-sm rounded-lg">
                            ...
                            delete the event, teams, matches, and polls.
                        </div>
                        <p className="text-sm">
                            Type{" "}
                            <span className="font-bold">{event?.name}</span> to
                            confirm.
                        </p>
                        <Input
