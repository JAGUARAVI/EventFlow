...
export default function Dashboard() {
    ...
    const canCreate =
        profile && ["admin", "club_coordinator"].includes(profile.role);
        ...
        (async () => {
            ...
            if (extra.length) {
                const { data } = await supabase
                    .from("events")
                    .select(
                        "id, name, type, event_types, visibility, created_at",
                    )
                    .in("id", extra);
                more = data || [];
            }
                            ...
                            >
                                ...
                                <CardBody className="py-4">
                                    ...
                                    <div className="flex flex-wrap gap-1">
                                        {Array.isArray(e.event_types) &&
                                        e.event_types.length > 0
                                            ? e.event_types.map((t) => (
                                                  <span
                                                      key={t}
                                                      className="text-xs px-2 py-1 rounded bg-default-100 text-default-500"
                                                  >
                                                      {t}
                                                  </span>
                                              ))
    ...
    return (
        <Card className="border-none shadow-sm hover:shadow-md transition-shadow">
            <CardBody className="p-4 flex flex-col items-center justify-center gap-2">
                <div
                    className={`p-2 rounded-full bg-${color}/10 text-${color}`}
                >
                    {Icon ? (
